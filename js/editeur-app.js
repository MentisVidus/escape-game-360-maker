...
function ensureLocalDraftStatusBar() {
    ...
    document.getElementById("local-draft-save-now").addEventListener("click", async function () {
        try {
            ...
            alert("Echec du snapshot local : " + (e && e.message ? e.message : String(e)));
        }
        await refreshLocalDraftStatusUi();
    });
    document.getElementById("local-draft-clear").addEventListener("click", async function () {
        if (!confirm("Effacer les brouillons locaux de cet onglet ?")) return;
        try {
...
async function refreshLocalDraftStatusUi() {
    ...
    try {
        ...
        if (failText) {
            failText.textContent = "Brouillon local indisponible (IndexedDB bloquee ou non supportee).";
    ...
    var msg = "Stockage: " + used + " Mo / " + quota + " Mo (" + ratio + "%)";
    if (info.warnLevel === "high") {
        msg += " - Alerte forte (>=90%): pensez au mode leger.";
...
async function maybeRestoreLocalDraftOnStartup() {
    ...
    if (incompatible.length > 0) {
        var purgeOld = confirm(
            "Des brouillons locaux d'une ancienne version ont ete detectes (" +
                incompatible.length +
    ...
    for (var i = 0; i < max; i++) {
        ...
        if (d.lightMode) flags.push("leger");
        lines.push(
            i +
                1 +
                ". " +
                (d.projectTitleSnapshot || "(Sans titre)") +
                " - " +
    ...
    lines.push("");
    lines.push("Entrer un numero pour restaurer (laisser vide pour ignorer).");
    var pick = prompt("Brouillons locaux detectes:\n\n" + lines.join("\n"), "1");
    ...
    if (restored.missingMedia && restored.missingMedia.length > 0) {
        alert(
            "Restauration partielle: " +
                ...
                " media(s) manquant(s). Verifiez les champs concernes."
        );
    }
}
