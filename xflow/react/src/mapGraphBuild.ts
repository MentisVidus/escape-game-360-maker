...
export type EditorScene = {
  ...
  title?: string;
  scTitle?: string;
  /** Scène réservée éditeur (file d’attente carte / orphelins) — filtrée à l’export joueur. */
  editorOnly?: boolean;
  media?: {
    panoramaUrl?: string;
    ambiance?: {
...
export function sceneIsEditorOnly(scene: EditorScene | undefined): boolean {
  if (!scene || typeof scene !== "object") return false;
  const eo = (scene as { editorOnly?: unknown }).editorOnly;
  return eo === true || eo === 1 || String(eo).toLowerCase() === "true";
}

/** Index de la première scène « jouable » (hors file d’attente carte). */
export function firstPlayableSceneIndex(project: EditorProject | null): number {
...
export function sceneTitleForGraph(
  ...
  if (sceneIsEditorOnly(scene)) {
    return lang === "en" ? "Unassigned (map)" : "Orphelins (carte)";
  }
...
export function filterProjectForNarrationSkeleton(
  ...
  for (let si = 0; si < scenes.length; si++) {
    const s = scenes[si];
    if (!s || sceneIsEditorOnly(s)) continue;
...
function computeFullViewPositionsByDepth(project: EditorProject): Record<string, Pos> {
  ...
  const entryIdx = firstPlayableSceneIndex(project);
  const entryKey = sceneKey(scenes[entryIdx], entryIdx);
...
function buildGraphFull(
  ...
  scenes.forEach((scene, si) => {
    ...
    const isStaging = sceneIsEditorOnly(scene);
  ...
  scenes.forEach((scene, si) => {
    const sk = sceneKey(scene, si);
    const isStagingHs = sceneIsEditorOnly(scene);
    const sceneNodeId = sceneKeyToRfId[sk];
    if (!isStagingHs && !sceneNodeId) return;
    const p = posByKey[sk] || { sx: 50, sy: 50 + si * 280 };
    const sx = p.sx;
    const sy = p.sy;
    const hotspots = Array.isArray(scene.hotspots) ? scene.hotspots : [];
...
function buildGraphFocus(
  ...
  if (!resolved) {
    ...
    resolved = { scene: scenes[fi], index: fi, key: sceneKey(scenes[fi], fi) };
  }
  ...
  uniqueTargets.forEach((tid, ti) => {
    ...
    if (!meta) return;
    if (sceneIsEditorOnly(meta.scene)) return;
  ...
  if (project.useGlobalAudio && project.globalMusic && String(project.globalMusic.url || "").trim()) {
    ...
    if (!isStagingFocus) {
      ...
      pushMapMetaEdge(edges, `hs:${activeKey}:0`, gid);
    }
  }
...
function buildGraphTree(project: EditorProject, lang: EditorLang, nodes: Node[], edges: Edge[]): void {
  ...
  function placeScene(sk: string, x: number, yCenter: number): { right: number; rootSceneRfId: string | null } {
    ...
    const hsList = Array.isArray(meta.scene.hotspots) ? meta.scene.hotspots : [];
    const hideSceneShell = sceneIsEditorOnly(meta.scene);
    const sceneRfId = `sc:${sk}`;
    const hsColX = hideSceneShell ? 28 : HOTSPOT_DX;
    if (!hideSceneShell) {
    ...
    hsList.forEach((hs, i) => {
      ...
      if (hsSfx) {
        ...
        nodes.push({
          id: rsId,
          type: "mapResource",
          position: { x: x + hsColX + 240, y: hy + 10 },
          data: {
            kind: "resource",
            label: lang === "en" ? "Hotspot SFX" : "SFX hotspot",
            resourceType: "hotspotSfx",
      ...
      if (flatSel.length > 0) {
        ...
        for (let ri = 0; ri < flatSel.length; ri++) {
          ...
          nodes.push({
            id: choiceId,
            type: "mapSelectorChoice",
            position: { x: x + hsColX + CHOICE_OFFSET_X, y: cy },
            draggable: true,
            selectable: true,
            data: {
              kind: "selectorChoice",
          ...
          if (row.sfx) {
            ...
            nodes.push({
              id: sfxId,
              type: "mapResource",
              position: { x: x + hsColX + CHOICE_OFFSET_X + 200, y: cy + 6 },
              data: {
                kind: "resource",
                label: lang === "en" ? "Choice SFX" : "SFX choix",
                resourceType: "choiceSfx",
          ...
          subtreeRight = Math.max(subtreeRight, x + hsColX + CHOICE_OFFSET_X + 170);
      ...
      if (targets.length === 0) {
        subtreeRight = Math.max(subtreeRight, x + hsColX + 200);
        return;
      }
  ...
  scenes.forEach((scene, si) => {
    ...
    if (sceneIsEditorOnly(scene)) {
      placeScene(sk, orphanBaseX, orphanY + orphanIndex * ORPHAN_STEP_Y);
      orphanIndex++;
      return;
    }
    placedSceneKeys.add(sk);
...
export function buildProjectMapGraph(
  ...
  let k = activeKey;
  if (k == null || k === "") {
    const fi = firstPlayableSceneIndex(workProject);
    k = sceneKey(scenesList[fi], fi);
  }
