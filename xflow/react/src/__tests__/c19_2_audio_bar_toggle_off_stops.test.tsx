...
describe("c19_2_audio_bar_toggle_off_stops", () => {
  it("mute panneau → stopAll({ clearLastSfx: false }) puis réactiver rejoue global", () => {
    const store = createNodalProjectStore();
    const scene: SceneNode = {
      id: asSceneNodeId("scn-mute"),
      nodeType: "scene",
      sceneId: "mute",
      label: "S",
    ...
    const muteBtn = Array.from(container.querySelectorAll("button")).find((b) => b.textContent?.includes("Couper"));
    expect(muteBtn).toBeTruthy();
    act(() => {
      (muteBtn as HTMLButtonElement).dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });

    expect(stopAllSpy).toHaveBeenCalledWith({ clearLastSfx: false });

    const playSpy = vi.spyOn(getAudioChannelsService(), "play");
    const unmute = Array.from(container.querySelectorAll("button")).find((b) => b.textContent?.includes("Réactiver"));
    expect(unmute).toBeTruthy();
    act(() => {
      (unmute as HTMLButtonElement).dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });
