...
import type { MediaNode, MsgActionNode, SceneNode } from "../model/nodes";
...
describe("c19_2_audio_bar_sfx_on_hotspot_click", () => {
  it("clic hotspot interactif → play sfx + lastSfx", () => {
    const store = createNodalProjectStore();
    const scene: SceneNode = {
      id: asSceneNodeId("scn-sfx"),
      nodeType: "scene",
      sceneId: "sfx-scene",
      label: "S",
    ...
    const msg: MsgActionNode = {
      id: asActionNodeId("act-msg-sfx"),
      nodeType: "action",
      actionType: "msg",
      label: "M",
      payload: { copy: { bodyHtml: "<p>x</p>", buttonLabel: "OK" } },
      sfx: { url: "https://example.com/sfx1.mp3", volume: 0.7 },
      visibility: VIS,
    };
    ...
    const hs = injectHotspotEl(body, scene.id, 0);
    act(() => {
      hs.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, clientX: 10, clientY: 10 }));
    });

    expect(playSpy).toHaveBeenCalledWith(
      "sfx",
      "https://example.com/sfx1.mp3",
      expect.objectContaining({ volume: 0.7, nodeId: msg.id })
    );
    expect(getAudioChannelsService().getState("sfx").lastSfx?.url).toContain("sfx1.mp3");
  });
});
