...
<script>
    ...
    // Fonction qui exécute la récompense (après un code ou une énigme)
    function executeReward(args, hsDiv) {
        if(args.action === 'scene') { 
            if(args.transTxt) { 
                afficherPopup("", args.transTxt, args.transBtn || "Continuer", function(){ viewer.loadScene(args.target); }); 
            } else { 
    ...
    // Le Cerveau du jeu : gère ce qu'il se passe quand on clique sur un point
