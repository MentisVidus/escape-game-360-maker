...
- **Démo d'accueil — nœud image par défaut** — La scène par défaut (à l'ouverture de la carte sans projet chargé) contient 2 scènes + 2 actions (msg + goto). Ajouter **un nœud image par scène** (média), relié à la scène par un **edge meta** (`scene → media-image`), avec l'URL par défaut = panorama placeholder grille (`DEFAULT_MEDIA_IMAGE_PLACEHOLDER_URL`, jsDelivr). Permet l'aperçu 360° immédiat via `resolveScenePanoramaDisplayUrl`. Voir `xflow/react/src/view/demoProject.ts`. ⇒ *livré*.
...
- **Inventaire virtuel pour l'aperçu « comme en jeu » (C18.5+)** — Aujourd'hui la preview C18.5 est volontairement read-only (sans simulation de l'inventaire ni des visibility-rules). À terme, un **inventaire virtuel** côté carte permettrait : tester les **REQ rewards** (visualiser ce qui est donné à la résolution d'une énigme), tester les **règles de visibilité** des choix selector / hotspots conditionnés à un objet (`visibleIf` / `requiredItems`), et valider les **transitions conditionnelles**. Conception possible : panneau latéral « Inventaire test » dans `<ScenePreviewModal>` (cocher / décocher des objets du catalogue `state.objects`), `<PlayerPreviewOverlay>` consulte cet état au lieu d'ignorer les conditions. **Chantier dédié** (pas C18.5.3, qui reste audio + polish). Probablement après le menu flottant S-BOX.
